...
CREATE PROC sp_Propped_Search_by_Name_and_Filters
@name varchar(50),
...
CREATE PROC sp_Search_by_Name_and_Category_and_Filters
@name varchar(50),
@category int,
@list AS Attribute_Values_List READONLY
AS
BEGIN 
	...
	OPEN myCursor
		...
		BEGIN
			...
			a.id_attribute = @current and
			p.name_product like '%' + @name + '%' and
			p.id_category_product = @category and
			pa.value_productattribute in (SELECT value from @list where id = @current)
...
CREATE PROC sp_Propped_Search_by_Name_and_Category_and_Subcategory_and_Filters
@name varchar(50),
@category int,
@subcategory int,
@list AS Attribute_Values_List READONLY
...
BEGIN 
	...
	OPEN myCursor
		...
		BEGIN
			...
			p.name_product like '%' + @name + '%' and
			p.id_category_product = @category and
			p.id_subcategory_product = @subcategory and
			pa.value_productattribute in (SELECT value from @list where id = @current)
...
CREATE PROC sp_Propped_Search_by_Category_and_Filters
@category int,
@list AS Attribute_Values_List READONLY
AS
BEGIN 
...
CREATE PROC sp_Propped_Search_by_Category_and_Subcategory_and_Filters
@category int,
@subcategory int,
@list AS Attribute_Values_List READONLY
AS
