...
Product.findByParams = (name, idCategory, idSubcategory, filters, result) => {
  ...
  let query = `SELECT DISTINCT p.* FROM Product_Propped p WHERE (p.color_product like '%${filters[0]}%') and (p.size_product like '%${filters[3]}%') and (p.occasion_product like '${filters[2]}') and ${filters[1]} and p.name_product like '%${name}%' and ${idCategory} and ${idSubcategory}`;
  console.log(query);
  sql.query(
    query,
    (err, res) => {
