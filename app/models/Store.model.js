...
const Store = function (store) {
  ...
  this.description_store = store.description_store;
};

Store.create = (newStore, result) => {
  sql.query(
    `INSERT INTO Store_Propped VALUES('${newStore.code_store}','${newStore.name_store}','${newStore.registry_store}','${newStore.website_store}','${newStore.phone_store}','${newStore.postal_code_store}','${newStore.address_store}','${newStore.city_store}','${newStore.state_store}', '${newStore.country_store}', '${newStore.image_store}', '${newStore.description_store}')`,
    (err, res) => {
      if (err) {
        result(err, null);
        return;
...
Store.updateByCode = (code, store, result) => {
  ...
  sql.query(
    `UPDATE Store_Propped SET name_store = '${store.name_store}', registry_store = '${store.registry_store}', website_store = '${store.website_store}', phone_store= '${store.phone_store}', postal_code_store= '${store.postal_code_store}', address_store= '${store.address_store}', city_store= '${store.city_store}', state_store= '${store.state_store}', country_store= '${store.country_store}', image_store = '${store.image_store}' WHERE code_store = '${code}'`,
    (err, res) => {
      if (err) {
